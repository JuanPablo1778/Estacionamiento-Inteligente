...
#        print(distancia3)
        if distancia < 6:
            ...
            sql2 = "UPDATE cajones SET estatus = %s  WHERE id = 1"
            cur.execute(sql2,("Ocupado"))
            conexion.commit()

        else:

            sql2 = "UPDATE cajones SET estatus = %s  WHERE id = 1"
            cur.execute(sql2,("Disponible"))
            conexion.commit()

            LED3.off()
        if distancia2 < 6:

            sql2 = "UPDATE cajones SET estatus = %s  WHERE id = 2"
            cur.execute(sql2,("Ocupado"))
            conexion.commit()
        ...
        if distancia3 < 6:
            ...
            sql2 = "UPDATE cajones SET estatus = %s  WHERE id = 3"
            cur.execute(sql2,("Ocupado"))
            conexion.commit()
